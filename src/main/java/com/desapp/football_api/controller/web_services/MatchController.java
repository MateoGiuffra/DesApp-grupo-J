...
@Tag(name = "Matches", description = "Get matches by team")
@AllArgsConstructor
public class MatchController {
    ...
    public ResponseEntity<List<Match>> getMatchesByTeam(
        ...
        return ResponseEntity.ok(teamService.getMatchesByTeamId(teamId, matchType, matchLocation));
    }

    @Operation(
            summary = "Get matches by team",
            description = "Returns the list of matches for the given team name. You can filter by match type (all/past/upcoming) and by location (all/home/away)."
    )
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "List of matches returned",
                    content = @Content(
    ...
    public ResponseEntity<List<Match>> getMatchesByTeamName(
            @Parameter(description = "Team name (e.g., Barcelona)")
        ...
        return ResponseEntity.ok(teamService.getMatchesByTeamName(name, matchType, matchLocation));
    }
